...
@section('content')
     ...
     <div class="row">
        ...
        <div class="col-md-8 offset-sm-0">
            <h3 class="text-center mt-3 form_title">{{__('apply for a loan')}} : {{__('payment dates')}} </h3>
            <hr>
            ...
            <div class="container">
                ...
                <form method="post" action="{{route('three-installment-loan.handle')}}" >
                    @csrf

                  <div class="form-row">

                        <div class="col-md-12 mt-4 installment_payback_date" id="first_installment_payback_date">
                        ...
                        <div class="col-md-12 mt-5 installment_payback_date">
                            <label class="form_text">{{__('third installment payback date')}}</label>
                            <div class="input-group">
                                <div class="input-group-prepend">
                                    <span class="input-group-text"><i class="far fa-calendar-alt"></i></span>
                                </div>
                                <input type="text" data-date-format="yyyy-mm-dd" id="datepickerThirdInstallment" name="thirdInstallmentPayBackDate"
                                       value="{{old('thirdInstallmentPayBackDate')}}"
                                       class="form-control {{ $errors->has('thirdInstallmentPayBackDate') ? ' is-invalid' : '' }}" />

                                @if ($errors->has('thirdInstallmentPayBackDate'))
                                    <span class="invalid-feedback" role="alert">
                                  <strong>{{ $errors->first('thirdInstallmentPayBackDate') }}</strong>
                                 </span>
                                @endif
                            </div>
                        </div>
                ...
                <a type="button" href="{{route('choose-number-of-installments.show')}}"
...
@endif
 ...
 $('#datepickerFirstInstallment').datepicker({
    weekStart: 1,
    daysOfWeekDisabled: [0, 1, 6], //prevent selection of weekends
...
//$('#datepickerFirstInstallment').datepicker("setDate", new Date());
 ...
 //date picker for payback date
 $('#datepickerThirdInstallment').datepicker({
    weekStart: 1,
